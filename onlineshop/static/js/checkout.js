    ...
    $('.order-form').on('submit', function (e) {
        ...
        if (merchant_id !== '') {
            IMP.request_pay({
                ...
                if (request.success) {
                    var msg = '결제가 완료되었습니다.';
                    msg += '고유 ID : ' + request.imp_uid;
                    // 결제 완료후 보여줄 메시지
                    ImpTransaction(e, order_id, request.merchant_uid, request.imp_uid, request.paid_amount);
                } else {
                    var msg = '결제에 실패하였습니다.';
                    msg += "에러내용 : " + request.error_msg;
                    console.log(msg)
                }
...
function ImpTransaction(e, order_id, merchant_id, imp_id, amount) {
    ...
    request.done(function (data) {
        if (data.works) {
            $(location).attr('href', location.origin+order_complete_url+'?order_id='+order_id) // created.html로 보냄
        }
    });
    request.fail(function (jqXHR, textStatus) {
        if (jqXHR.status == 404) {
